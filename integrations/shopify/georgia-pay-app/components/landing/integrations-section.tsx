      ...
      <Stagger className="grid gap-8 md:grid-cols-3">
        {s.platforms.map((p, i) => (
          <StaggerItem key={p.name} className="h-full">
            <Link href={route(integrationRouteKey(p.name))} className="block h-full">
              <motion.div
                whileHover={{ y: -8 }}
              ...
              >
                ...
                <span className="mt-6 text-sm font-medium text-accent-cyan group-hover:underline">
                  {s.learnMore} →
                </span>
                <motion.div
                  className="mt-4 h-1 w-12 rounded-full bg-gradient-to-r from-accent-cyan to-accent-violet"
                  initial={{ scaleX: 0 }}
