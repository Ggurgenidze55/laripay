...
export function databaseMisconfiguredUserMessage(hint?: string): string {
  if (!hint) return 'Database is unavailable. Try again in a few seconds.';
  if (hint.includes('Build-time localhost') || hint.includes('On Vercel, set DATABASE_URL')) {
  ...
  if (hint.includes('Local Postgres') || hint.includes('localhost:5433')) {
